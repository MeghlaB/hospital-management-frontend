...
function AppointmentList() {
  ...
  if (isLoading) {
    return <p className="text-center text-gray-500 mt-10">Loading appointments...</p>;
  }
  ...
  // Filter appointments based on selected specialization
          ...
          <table className="min-w-full table-auto border-collapse">
            ...
            <tbody>
              {filteredAppointments.length === 0 ? (
                <tr>
                  <td colSpan="6" className="text-center text-red-500 text-lg font-semibold">
                ...
                filteredAppointments.map((appointment) => (
                  <tr key={appointment._id}>
                    ...
                    <td className="border px-4 py-2">{appointment.specialization}</td>
                    <td className="border px-4 py-2">{appointment.date}</td>
                    <td className="border px-4 py-2">{appointment.time}</td>
                    <td className="border px-4 py-2">{appointment.name}</td>
                    <td className="border px-4 py-2 text-center">
                      <Link to={`/appointments/${appointment._id}`}>
                        <button className="w-full bg-[#00786F] text-white py-2 rounded-xl hover:bg-[#00786F] transition">
                          View Details
                        </button>
                      </Link>
                    </td>
