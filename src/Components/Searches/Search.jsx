...
export default function DoctorSearchSection() {
  ...
  const handleSearch = debounce(async () => {
    ...
    try {
      ...
      setError("An error occurred while fetching doctors.",error.message);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="bg-blue-800 text-white p-6 rounded-md w-full max-w-5xl mx-auto shadow-md">
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {loading ? (
          <div className="text-white text-center col-span-3">Loading...</div>
        ) : doctors.length === 0 ? (
          <div className="text-white text-center col-span-3">No doctors found.</div>
        ) : (
          doctors.map((doc) => (
